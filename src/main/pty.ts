...
function getPowerShellBootstrapCommand(): string {
  return [
    ...
    "  if ($displayPath -match '^\\\\\\\\(?:wsl\\.localhost|wsl\\$)\\\\[^\\\\]+(?<path>\\\\.*)$') {",
    "    $displayPath = $Matches['path']",
    '  }',
    "  return \"PS $displayPath> \"",
    '}',
    'function global:__bridgegit_resolve_wsl_launch_context {',
    '  $location = Get-Location',
    '  $providerPath = $location.ProviderPath',
    '  if (-not $providerPath) {',
    '    return $null',
    '  }',
    "  if ($providerPath -match '^(?<drive>[A-Za-z]):\\\\(?<suffix>.*)$') {",
    "    $linuxPath = if ($Matches['suffix']) {",
    "      '/mnt/' + $Matches['drive'].ToLowerInvariant() + '/' + ($Matches['suffix'] -replace '\\\\', '/')",
    '    } else {',
    "      '/mnt/' + $Matches['drive'].ToLowerInvariant()",
    '    }',
    '    return @{ LinuxPath = $linuxPath; Distro = $null }',
    '  }',
    "  if ($providerPath -match '^\\\\\\\\(?:wsl\\.localhost|wsl\\$)\\\\(?<distro>[^\\\\]+)(?<path>\\\\.*)?$') {",
    "    $linuxPath = if ($Matches['path']) { ($Matches['path'] -replace '\\\\', '/') } else { '/' }",
    "    return @{ LinuxPath = $linuxPath; Distro = $Matches['distro'] }",
    '  }',
    '  return $null',
    '}',
    'function global:wsl {',
    ...
    '    return',
    '  }',
    '  $launchContext = __bridgegit_resolve_wsl_launch_context',
    '  if (-not $launchContext) {',
    '    & wsl.exe @args',
    '    return',
    '  }',
    "  $explicitDistroIndex = [Array]::IndexOf($args, '-d')",
    "  if ($explicitDistroIndex -lt 0) {",
    "    $explicitDistroIndex = [Array]::IndexOf($args, '--distribution')",
    '  }',
    '  $hasExplicitDistro = $explicitDistroIndex -ge 0',
    '  $wslArgs = New-Object System.Collections.Generic.List[string]',
    '  if ($launchContext.Distro -and -not $hasExplicitDistro) {',
    "    $wslArgs.Add('--distribution')",
    '    $wslArgs.Add([string]$launchContext.Distro)',
    '  }',
    "  $wslArgs.Add('--cd')",
    '  $wslArgs.Add([string]$launchContext.LinuxPath)',
    '  foreach ($arg in $args) {',
    '    $wslArgs.Add([string]$arg)',
    '  }',
    '  & wsl.exe @wslArgs',
    '}',
  ].join('; ');
